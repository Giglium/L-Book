...
adminMainWindow::adminMainWindow( const QSharedPointer < account > user, QWidget *parent)
...
void adminMainWindow::on_loanButton_clicked()
{
    emit openLoan( logUser );
}

void adminMainWindow::on_saveButton_clicked()
{
   emit saveData();
}
...
void adminMainWindow::saveResult( const bool &result ){
    if( result )
        QMessageBox::information( this, tr( "Saved" ), tr( "All data have been saved!" ) );
}

void adminMainWindow::on_clearButton_clicked(){
    QMessageBox messageBox( QMessageBox::Information, tr( "Unistall" ), tr( " You really want to delete all the local data and close the program ? " ), QMessageBox::Yes|QMessageBox::No );
...
void adminMainWindow::on_userButton_clicked(){
    emit openUser( logUser );
}
