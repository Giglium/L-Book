...
modifyUser::modifyUser(const QSharedPointer<account> thisUser, const bool type,
                       QWidget *parent)
  ...
  ui->usernameLine->setValidator(
      new QRegExpValidator(QRegExp("[A-Za-z0-9_]*"), this));

  if (user.isNull()) {
    ui->dateEdit->setDate(QDate::currentDate());
    ui->nextButton->setEnabled(false);
    QObject::connect(ui->nameLine, SIGNAL(textChanged(const QString &)), this,
                     SLOT(checkLineEdits(const QString &)));
    QObject::connect(ui->surnameLine, SIGNAL(textChanged(const QString &)),
                     this, SLOT(checkLineEdits(const QString &)));
    QObject::connect(ui->usernameLine, SIGNAL(textChanged(const QString &)),
                     this, SLOT(checkLineEdits(const QString &)));
    QObject::connect(ui->passwordLine, SIGNAL(textChanged(const QString &)),
                     this, SLOT(checkLineEdits(const QString &)));
    ...
    if (mode) {
      ...
      ui->infoLabel->setText(
          tr("I have not found any admin. So please register your account:"));
      ui->noteLabel->setText(
          tr("Account and Information field can't be left blank. Username "
             "cannot be changed after registration."));
    }
  } else {
    ui->infoLabel->setText(tr("Here you can modify some user data:"));
    ui->noteLabel->setText(
        tr("If you insert a password it will subscribe the old one."));
    ...
    if (!user->p_permission().isNull()) {
      ui->loanLine->setText(
          QString::number(user->p_permission()->showMaxLoan()));
...
void modifyUser::userResult(const bool &result) {
  if (result) {
    ...
    QMessageBox::critical(this, tr("Error"),
                          tr("This username already exist."));
  }
}
...
modifyUser::~modifyUser() { delete ui; }

void modifyUser::on_ExitButton_clicked() { QDialog::reject(); }

void modifyUser::on_nextButton_clicked() {
  if (user.isNull()) {
    QSharedPointer<userinfo> info(new userinfo(
        ui->nameLine->text(), ui->surnameLine->text(), ui->dateEdit->date(),
        ui->classLine->text(), ui->cellLine->text(), ui->mailLine->text()));
    QSharedPointer<account_permission> permission(new account_permission(
        ui->comboBox->currentIndex() + 1, ui->loanLine->text().toInt()));

    if (!ui->usernameLine->text().isEmpty() &&
        !ui->passwordLine->text().isEmpty()) {
      QSharedPointer<username> name(
          new username(ui->usernameLine->text(),
                       utility::encrypt(ui->passwordLine->text())));
    ...
    if (user->p_permission().isNull()) {
      user->setPermission(ui->comboBox->currentIndex() + 1,
                          ui->loanLine->text().toInt());
    } else {
    ...
    if (user->p_user().isNull() && !ui->usernameLine->text().isEmpty()) {
      if (!ui->passwordLine->text().isEmpty()) {
        emit sendUsername(user, ui->usernameLine->text(),
                          utility::encrypt(ui->passwordLine->text()));
        ok &= false;
      } else {
        ui->passwordLine->setStyleSheet("background: rgb(255, 0, 0);");
        QMessageBox::critical(this, tr("Error"),
                              tr("you need also to setup a password."));
        ok &= false;
      }
    } else {
      if (!ui->passwordLine->text().isEmpty())
        user->p_user()->setPassword(utility::encrypt(ui->passwordLine->text()));
    }
    if (ok) QDialog::accept();
  }
}
