...
mainWindow::mainWindow(QWidget *parent)
  ...
  connect(ui->usernameLine, SIGNAL(textChanged(const QString &)), this,
          SLOT(checkLineEdits(const QString &)));
  connect(ui->passwordLine, SIGNAL(textChanged(const QString &)), this,
          SLOT(checkLineEdits(const QString &)));
}
...
void mainWindow::on_comboBox_currentIndexChanged(int index) {
  if (index) emit changeLanguage(index);
}
...
void mainWindow::languageResult(const bool &result) {
  if (result) {
    ...
    QMessageBox::critical(this, tr("Error"),
                          tr("Language files don't found. This program is "
...
void mainWindow::badLogin() {
  QMessageBox::critical(this, tr("Bad login"),
                        tr("Wrong username or password"));
}

void mainWindow::on_nextButton_clicked() {
  emit loginRequest(ui->usernameLine->text(),
                    utility::encrypt(ui->passwordLine->text()));
  ui->passwordLine->setText("");
}

void mainWindow::on_booksButton_clicked() { emit publicationListRequest(true); }

void mainWindow::on_periodicalsButton_clicked() {
  emit publicationListRequest(false);
}

void mainWindow::on_allButton_clicked() { emit publicationListRequest(true); }
