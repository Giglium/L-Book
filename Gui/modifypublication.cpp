...
modifyPublication::modifyPublication(const QSharedPointer<publication> publi,
                                     QWidget *parent)
  ...
  QObject::connect(ui->titleLine, SIGNAL(textChanged(const QString &)), this,
                   SLOT(checkLineEdits(const QString &)));
  QObject::connect(ui->isbnLine, SIGNAL(textChanged(const QString &)), this,
                   SLOT(checkLineEdits(const QString &)));
  QObject::connect(ui->poliLine_2, SIGNAL(textChanged(const QString &)), this,
                   SLOT(checkLineEdits(const QString &)));
  QObject::connect(ui->poliLine_10, SIGNAL(textChanged(const QString &)), this,
                   SLOT(checkLineEdits(const QString &)));
  QObject::connect(ui->poliLine_12, SIGNAL(textChanged(const QString &)), this,
                   SLOT(checkLineEdits(const QString &)));
  QObject::connect(ui->dateEdit, SIGNAL(dateChanged(QDate)), this,
                   SLOT(checkLineEdits(QDate)));
  ...
  if (currentPublication.isNull()) {
    ...
    if (!publi->p_permission().isNull()) {
      ...
      ui->nCopyLine->setText(
          QString::number(publi->p_permission()->showCopyNumber()));
    }
    ...
    QSharedPointer<periodic> magazine =
        qSharedPointerDynamicCast<periodic>(publi);
    if (!magazine.isNull()) {
...
modifyPublication::~modifyPublication() { delete ui; }

void modifyPublication::on_exitButton_clicked() { QDialog::reject(); }
...
void modifyPublication::on_publiBox_currentIndexChanged(int index) {
  ...
  if (index == 2) {
    ui->noteLabel->setText(
        tr("Title, Release Date, Years, ISBN13 or ISSN and "
           "Number are always required."));
...
void modifyPublication::checkLineEdits(const QDate &) { checkLineEdits(""); }

void modifyPublication::on_saveButton_clicked() {
  if (currentPublication.isNull()) {
    if (ui->publiBox->currentIndex() == 1) {
      if (utility::checkISBN13(ui->isbnLine->text())) {
        currentPublication = QSharedPointer<publication>(new book(
            ui->poliLine_2->text(), ui->plotLine->toPlainText(),
            ui->titleLine->text(), ui->publishingLine->text(),
            ui->isbnLine->text(), ui->subtitleLine->text(),
            ui->genreLine->text(), ui->dateEdit->date(),
            ui->poliLine_12->text(), ui->poliLine_8->text(),
            ui->poliLine_10->text().toInt(), ui->poliLine_6->text().toInt(),
            ui->poliLine_4->text().toInt()));
      } else {
        isbnError();
      }
    }
      ...
      if (!ui->isbnLine->text().isEmpty()) {
        if (utility::checkISBN13(ui->isbnLine->text())) {
          if (utility::checkAllInteger(ui->poliLine_12->text()))
            currentPublication = QSharedPointer<publication>(new periodic(
                ui->poliLine_2->text(), ui->poliLine_12->text().toInt(),
                ui->poliLine_10->text().toInt(), ui->titleLine->text(),
                ui->publishingLine->text(), ui->isbnLine->text(),
                ui->dateEdit->date(), ui->subtitleLine->text(),
                ui->genreLine->text()));
          else
            currentPublication = QSharedPointer<publication>(
                new periodic(ui->poliLine_2->text(), ui->poliLine_12->text(),
                             ui->poliLine_10->text().toInt(),
                             ui->titleLine->text(), ui->publishingLine->text(),
                             ui->isbnLine->text(), ui->dateEdit->date(),
                             ui->subtitleLine->text(), ui->genreLine->text()));
        } else {
          isbnError();
        }
      }
    } else {
      if (!ui->poliLine_2->text().isEmpty() &&
          utility::checkISSN(ui->poliLine_2->text())) {
        if (utility::checkAllInteger(ui->poliLine_12->text()))
          currentPublication = QSharedPointer<publication>(new periodic(
              ui->poliLine_2->text(), ui->poliLine_12->text().toInt(),
              ui->poliLine_10->text().toInt(), ui->titleLine->text(),
              ui->publishingLine->text(), ui->poliLine_2->text(),
              ui->dateEdit->date(), ui->subtitleLine->text(),
              ui->genreLine->text()));
        else
          currentPublication = QSharedPointer<publication>(
              new periodic(ui->poliLine_2->text(), ui->poliLine_12->text(),
                           ui->poliLine_10->text().toInt(),
                           ui->titleLine->text(), ui->publishingLine->text(),
                           ui->poliLine_2->text(), ui->dateEdit->date(),
                           ui->subtitleLine->text(), ui->genreLine->text()));
      } else {
        isbnError(false);
      }
    ...
    if (ui->typeCombo->currentIndex())
      currentPublication->setPermission(ui->typeCombo->currentIndex() - 1,
                                        ui->nCopyLine->text().toInt());

    if (!currentPublication.isNull()) emit savePublication(currentPublication);
  } else {
    ...
    QSharedPointer<book> manual =
        qSharedPointerDynamicCast<book>(currentPublication);
    if (!manual.isNull()) {
    ...
    QSharedPointer<periodic> magazine =
        qSharedPointerDynamicCast<periodic>(currentPublication);
    if (!magazine.isNull()) {
    ...
    if (ui->typeCombo->currentIndex()) {
      if (currentPublication->p_permission().isNull()) {
        ...
        currentPublication->p_permission()->setType(
            ui->typeCombo->currentIndex());
        currentPublication->p_permission()->setCopyNumber(
            ui->nCopyLine->text().toInt());
      }
    }
...
void modifyPublication::isbnError(const bool &mode) {
  if (mode) {
    ...
    ui->noteLabel->setText(
        tr("Sorry but the ISBN 13 that you have insert isn't valid."));
    ...
    ui->noteLabel->setText(
        tr("Sorry but the ISSN that you have insert isn't valid."));
    QMessageBox::critical(
        this, tr("Error"),
        tr("Sorry but the ISSN that you have insert isn't valid."));
  }
...
void modifyPublication::addResult(const bool &result) {
  if (result) {
    ...
    ui->isbnLine->setStyleSheet("background: rgb(255, 0, 0);");
    ui->noteLabel->setText(
        tr("There is already a publication with the ISBN 13 that you have "
           "insert. If you want to add a periodic you have to remember to "
           "insert also the ID."));
    QMessageBox::critical(
        this, tr("Error"),
        tr("There is already a publication with the ISBN 13 that you have "
           "insert.There is already a publication with the ISBN 13 that you "
           "have insert. If you want to add a periodic you have to remember to "
           "insert also the ID."));
  }
}
