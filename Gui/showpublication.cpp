...
showPublication::showPublication(const QSharedPointer<publication> publi,
...
showPublication::~showPublication() { delete ui; }
...
void showPublication::setPubli() {
  ...
  if (!magazine.isNull()) {
    ...
    ui->isbnLabel->setText(magazine->showISBN_13() + " " +
                           magazine->showID_number());
    ...
    ui->poliLabel_12->setText(magazine->showRomanActivityYears() + "  (" +
                              QString::number(magazine->showActivityYears()) +
                              ")");
    ui->plotBox->hide();
  }

  QSharedPointer<publication_permission> per =
      currentPublication->p_permission();
  ...
  if (!per.isNull()) {
    switch (per->showType()) {
      case 0:
        ...
        break;
      case 1:
        ui->typeLabel->setText(tr("Not Available"));
        break;
      case 2:
        ui->typeLabel->setText(tr("Available"));
        break;
    }
...
void showPublication::setTable(const loans &lends) {
  ...
  if (!logUser.isNull() && logUser->p_permission()->showType() == 3) {
    ...
    QStringList title;
    title << tr("Name") << tr("Surname") << tr("Class") << tr("Start Date")
          << tr("DeadLine") << tr("Redelivered");
...
void showPublication::addRow(const QSharedPointer<loan> lend) {
  ...
  if (ui->tableWidget->columnCount() == 6) {
    ...
    ui->tableWidget->setItem(
        row, colum,
        new QTableWidgetItem(lend->p_user()->p_info()->showClass()));
    ++colum;
  }
...
void showPublication::on_exitButton_clicked() { QDialog::reject(); }
...
void showPublication::on_deleteButton_clicked() {
  QMessageBox messageBox(QMessageBox::Information, tr("Attention"),
                         tr(" Attention please, this action is permanent and "
                            "can't be changed once you have saved. Are you "
                            "sure to delete this book and all his loan? "),
