...
makeLoan::makeLoan( const dataCollector &coll, const QSharedPointer < account > log , QWidget *parent)
...
void makeLoan::on_exitButton_clicked(){
    QDialog::reject();
}
...
void makeLoan::addRow( const QSharedPointer < account > user ){
    ...
    ui->userTable->setItem( row, 0, new QTableWidgetItem( user->p_info()->showName() ) );
    ui->userTable->setItem( row, 1, new QTableWidgetItem( user->p_info()->showSurname() ) );
    ui->userTable->setItem( row, 2, new QTableWidgetItem( user->p_info()->showClass() ) );
}
...
void makeLoan::addRow( const QSharedPointer < publication > publi ){
    ...
    ui->publicationTable->setItem( row, 0, new QTableWidgetItem( publi->showTitle() ) );
    ui->publicationTable->setItem( row, 1, new QTableWidgetItem( publi->showISBN_13() ) );
}
...
void makeLoan::on_publicationTable_cellDoubleClicked( int row, int ){
    showPublication bookView( currenPublications.showPublication( row ),  collector.p_loans() );
    bookView.exec();
}
...
void makeLoan::on_publiSearchButton_clicked(){
    ...
    if( utility::checkISSN( searchText ) )
        currenPublications<<collector.p_publications()->showPublicationsFromISSN( searchText );

    if( utility::checkISBN13( searchText ) )
        currenPublications<<collector.p_publications()->showPublicationsFromISBN13( searchText );

    if( ( searchText.size() == 18 || searchText.size() == 19 ) && utility::checkAllInteger( searchText ) )
        currenPublications<<collector.p_publications()->showPublication( searchText.remove(" ") );

    currenPublications<<collector.p_publications()->showPublicationsFromTitle( searchText );

    currenPublications<<collector.p_publications()->thisAuthorPublications( searchText );
...
void makeLoan::on_makeButton_clicked(){
    if( collector.p_loans()->makeLoan( collector.p_accounts()->showUser( ui->userTable->currentRow() ), collector.p_publications()->showPublication( ui->publicationTable->currentRow() ), ui->durationLine->text().toInt() ) )
        QDialog::accept();
    else
        QMessageBox::critical( this, tr( "Error" ), tr( "Please check the permission of the user or the publication. It wasn't possible to make loan." ) );
}
