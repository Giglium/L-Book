...
publicationList::publicationList(const QSharedPointer<library> list,
                                 const QSharedPointer<loans> loansList,
                                 const bool mode,
                                 const QSharedPointer<account> user,
                                 QWidget *parent)
  ...
  QStringList title;
  title << tr("Title") << tr("Subtittle") << tr("Author") << tr("Edition")
        << tr("Genre") << tr("ISBN 13");
  ui->bookTable->setHorizontalHeaderLabels(title);
  title.clear();
  title << tr("Title") << tr("Subtittle") << tr("Number") << tr("Genre")
        << tr("ISSN") << tr("ISBN 13");
...
publicationList::~publicationList() { delete ui; }
...
void publicationList::setTables() {
  ...
  ui->resultLabel->setText(
      QString::number(currentPublications.showPublicationNumber()));
...
void publicationList::addRow(const QSharedPointer<publication> publicatio) {
  ...
  if (!manual.isNull()) {
    ...
    ui->bookTable->setItem(rowB, 1,
                           new QTableWidgetItem(manual->showSubtitle()));
    ui->bookTable->setItem(rowB, 2, new QTableWidgetItem(manual->showAuthor()));
    ui->bookTable->setItem(
        rowB, 3, new QTableWidgetItem(QString::number(manual->showEdition())));
    ui->bookTable->setItem(rowB, 4, new QTableWidgetItem(manual->showGenre()));
    ui->bookTable->setItem(rowB, 5,
                           new QTableWidgetItem(manual->showISBN_13()));
  }
  ...
  if (!magazine.isNull()) {
    ...
    ui->periodicTable->setItem(rowP, 0,
                               new QTableWidgetItem(magazine->showTitle()));
    ui->periodicTable->setItem(rowP, 1,
                               new QTableWidgetItem(magazine->showSubtitle()));
    ui->periodicTable->setItem(
        rowP, 2, new QTableWidgetItem(QString::number(magazine->showNumber())));
    ui->periodicTable->setItem(rowP, 3,
                               new QTableWidgetItem(magazine->showGenre()));
    ui->periodicTable->setItem(rowP, 4,
                               new QTableWidgetItem(magazine->showISSN()));
    ui->periodicTable->setItem(rowP, 5,
                               new QTableWidgetItem(magazine->showISBN_13()));
  }
}
...
void publicationList::on_exitButton_clicked() { QDialog::reject(); }
...
void publicationList::on_searchButton_clicked() {
  ...
  if (utility::checkISSN(searchText))
    currentPublications << allPublications->showPublicationsFromISSN(
        searchText);

  if (utility::checkISBN13(searchText))
    currentPublications << allPublications->showPublicationsFromISBN13(
        searchText);

  if ((searchText.size() == 18 || searchText.size() == 19) &&
      utility::checkAllInteger(searchText))
    currentPublications << allPublications->showPublication(
        searchText.remove(" "));
...
void publicationList::on_bookTable_cellDoubleClicked(int row, int) {
  showPublication bookView(currentPublications.allBook().showPublication(row),
                           allLoans, logUser);

  QObject::connect(&bookView, SIGNAL(sendPath(loans, QString)), this,
                   SLOT(receivePath(loans, QString)));
  QObject::connect(&bookView,
                   SIGNAL(erasePublication(QSharedPointer<publication>)), this,
                   SLOT(removePublication(QSharedPointer<publication>)));
...
void publicationList::on_periodicTable_cellDoubleClicked(int row, int) {
  showPublication bookView(
      currentPublications.allPeriodic().showPublication(row), allLoans,
      logUser);
...
void publicationList::receivePath(const loans &lends, const QString &path) {
  emit sendPath(lends, path);
}

void publicationList::removePublication(
    const QSharedPointer<publication> publi) {
...
void publicationList::on_addButton_clicked() {
  ...
  QObject::connect(&newPublication,
                   SIGNAL(savePublication(QSharedPointer<publication>)), this,
                   SLOT(addPublication(QSharedPointer<publication>)));
  QObject::connect(this, SIGNAL(addPublicationResult(bool)), &newPublication,
                   SLOT(addResult(bool)));
