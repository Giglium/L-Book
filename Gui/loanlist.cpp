...
loanList::loanList( const dataCollector &coll, const QSharedPointer < account > user , QWidget *parent)
         :QDialog(parent), ui(new Ui::loanList)
{
    ...
    QStringList title;
    title<<tr( "Start Date" )<<tr( "DeadLine" )<<tr( "Redelivered")<<tr( "Day to end" )<<tr( "Title" )<<tr( "ISBN 13" )<<tr("Name")<<tr("Class");
...
void loanList::on_exitButton_clicked() {
    QDialog::reject();
}
...
void loanList::addRow( const QSharedPointer < loan > lend ){
    ...
    ui->tableWidget->setItem( row, 0, new QTableWidgetItem( lend->showCharStartDate() ) );
    if( !lend->isfinish() ){
        ui->tableWidget->setItem( row, 1, new QTableWidgetItem( lend->showCharEndDate() ) );
        ui->tableWidget->setItem( row, 3, new QTableWidgetItem( lend->showDaytoEnd() > 0 ? QString::number( lend->showDaytoEnd() ) : tr("Expired") ) );
    }
    else{
        ui->tableWidget->setItem( row, 2, new QTableWidgetItem( lend->showCharEndDate() ) );
        ui->tableWidget->setItem( row, 3, new QTableWidgetItem( tr("Redelivered") ) );
    }
    ui->tableWidget->setItem( row, 4, new QTableWidgetItem( lend->p_title()->showTitle() ) );
    ui->tableWidget->setItem( row, 5, new QTableWidgetItem( lend->p_title()->showISBN_13() ) );
    ui->tableWidget->setItem( row, 6, new QTableWidgetItem( lend->p_user()->p_info()->showName() + " " + lend->p_user()->p_info()->showSurname() ) );
    ui->tableWidget->setItem( row, 7, new QTableWidgetItem( lend->p_user()->p_info()->showClass() ) );
}
...
void loanList::on_searchButton_clicked(){
    ...
    if( utility::checkISSN( searchText ) ){
        publiSearch = collector.p_publications()->showPublicationsFromISSN( searchText );
        for( int i = 0; i < publiSearch.showPublicationNumber(); ++i )
            currentLoans<<collector.p_loans()->publicationLoans( publiSearch.showPublication( i ) );
    }

    if( utility::checkISBN13( searchText ) ){
        publiSearch = collector.p_publications()->showPublicationsFromISBN13( searchText );
        for( int i = 0; i < publiSearch.showPublicationNumber(); ++i )
            currentLoans<<collector.p_loans()->publicationLoans( publiSearch.showPublication( i ) );
    }

    publiSearch = collector.p_publications()->showPublicationsFromTitle( searchText );
    for( int i = 0; i < publiSearch.showPublicationNumber(); ++i )
        currentLoans<<collector.p_loans()->publicationLoans( publiSearch.showPublication( i ) );
