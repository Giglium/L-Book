...
showLoan::showLoan( const QSharedPointer < loan > lend, const QSharedPointer< account > logUser, QWidget *parent )
    ...
    QSharedPointer < book > manual = qSharedPointerDynamicCast < book > ( lend->p_title() );
    if( !manual.isNull() ){
    ...
    QSharedPointer < periodic > magazine = qSharedPointerDynamicCast < periodic > ( lend->p_title() );
    if( !magazine.isNull() ){
...
void showLoan::setInformatin(){
    ...
    if( currentLoan->isfinish() ){
        ...
        ui->endDayLabel->setText( currentLoan->showDaytoEnd() > 0 ? QString::number( currentLoan->showDaytoEnd() ) : tr("Expired") );
...
{
    QMessageBox messageBox( QMessageBox::Information, tr( "Attention" ), tr( " You really want to confirm the redelivery of this loan: " ), QMessageBox::Yes|QMessageBox::No );
...
void showLoan::on_exitButton_clicked()
{
    QDialog::reject();
}
...
{
    QMessageBox messageBox(QMessageBox::Information, tr( "Attention" ), tr( " You really want to delete this loan: " ), QMessageBox::Yes|QMessageBox::No);
    messageBox.setButtonText( QMessageBox::Yes, tr( "Yes" ) );
    messageBox.setButtonText( QMessageBox::No, tr( "No" ) );
    if( QMessageBox::Yes == messageBox.exec() ) {
