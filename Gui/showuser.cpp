...
showUser::showUser(const QSharedPointer<account> user,
                   const QSharedPointer<loans> lends,
...
void showUser::setTable(const loans &lends) {
  ...
  if (ui->tableWidget->columnCount() != 7) {
    ...
    QStringList title;
    title << tr("Title") << tr("Subtittle") << tr("ISBN 13") << tr("Start Date")
          << tr("DeadLine") << tr("Redelivered") << tr("Day to end");
...
void showUser::setClassTable(const loans &lends) {
  ...
  if (ui->tableWidget->columnCount() != 8) {
    ...
    QStringList title;
    title << tr("Name") << tr("Surname") << tr("Title") << tr("Subtittle")
          << tr("ISBN 13") << tr("Start Date") << tr("DeadLine")
          << tr("Redelivered");
    ui->tableWidget->setHorizontalHeaderLabels(title);
  }
...
void showUser::addRow(const QSharedPointer<loan> lend) {
  ...
  if (ui->tableWidget->columnCount() == 8) {
    ui->tableWidget->setItem(
        row, colum, new QTableWidgetItem(lend->p_user()->p_info()->showName()));
    ++colum;
    ui->tableWidget->setItem(
        row, colum,
        new QTableWidgetItem(lend->p_user()->p_info()->showSurname()));
    ++colum;
  }

  ui->tableWidget->setItem(row, colum,
                           new QTableWidgetItem(lend->p_title()->showTitle()));
  ++colum;
  ui->tableWidget->setItem(
      row, colum, new QTableWidgetItem(lend->p_title()->showSubtitle()));
  ++colum;
  ui->tableWidget->setItem(
      row, colum, new QTableWidgetItem(lend->p_title()->showISBN_13()));
  ++colum;
  ui->tableWidget->setItem(row, colum,
...
void showUser::setUserData(const QSharedPointer<account> user) {
  ...
  if (!user->p_permission().isNull()) {
    ...
    if (type == 0) ui->typeLabel->setText(tr("Banned"));

    if (type == 1) ui->typeLabel->setText(tr("Student"));

    if (type == 2) ui->typeLabel->setText(tr("Professor"));

    if (type == 3) ui->typeLabel->setText(tr("Admin"));

    ui->loansLabel->setText(
        QString::number(user->p_permission()->showFreedomLoan()));
    ui->lockedLabel->setText(
        QString::number(user->p_permission()->showLockedLoan()));
  }
}
...
void showUser::on_ExitButton_clicked() { QDialog::reject(); }
...
void showUser::on_comboBox_currentIndexChanged(int index) {
  if (index) {
    CurrentLoans = AllLoans
                       ->unorderClassroomLoans(
                           currentUser->p_info()->showClassList().at(index - 1))
                       .studentLoans();
...
void showUser::on_searchButton_clicked() {
  if (!ui->surnameEdit->text().isEmpty()) {
    if (ui->comboBox->currentIndex()) {
      CurrentLoans =
          AllLoans
              ->unorderClassroomLoans(currentUser->p_info()->showClassList().at(
                  ui->comboBox->currentIndex() - 1))
              .studentLoans()
              .surnameLoans(ui->surnameEdit->text());

      setClassTable(CurrentLoans);
    } else {
      QMessageBox::critical(this, tr("Error"),
                            tr("You have to select a class first"));
    }
  }
}
...
void showUser::on_modifyButton_clicked() {
  ...
  QObject::connect(
      &modify, SIGNAL(sendUsername(QSharedPointer<account>, QString, QString)),
      this, SLOT(receiveUsername(QSharedPointer<account>, QString, QString)));
  QObject::connect(this, SIGNAL(sendResult(bool)), &modify,
...
void showUser::on_banButton_clicked() {
  if (currentUser->p_permission()->showType() == 3) {
    QMessageBox::information(this, tr("Error"),
                             tr("The Admin can't be banned!"));
  } else {
    QMessageBox messageBox(
        QMessageBox::Information, tr("Attention"),
        tr(" Attention please, this action is permanent and can't be changed "
           "once you have saved. If the account don't have any pending loans "
           "it with be erased, if not it will be permanently banned. "),
        QMessageBox::Yes | QMessageBox::No);
    messageBox.setButtonText(QMessageBox::Yes, tr("Yes"));
    messageBox.setButtonText(QMessageBox::No, tr("No"));
    if (QMessageBox::Yes == messageBox.exec()) {
      if (AllLoans->userLoans(currentUser)
              .showUnfinishLoan()
              .showLoanNumber()) {
...
void showUser::receiveUsername(const QSharedPointer<account> user,
                               const QString &name, const QString &pass) {
  emit sendUsername(user, name, pass);
}

void showUser::receiveResult(const bool &result) { emit sendResult(result); }
