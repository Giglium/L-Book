...
userList::userList( const QSharedPointer < users > accounts, const QSharedPointer <loans> lend, const QSharedPointer <account> log , QWidget *parent)
...
void userList::on_exitButton_clicked(){
    QDialog::reject();
}
...
void userList::addRow( const QSharedPointer < account > user ){
    ...
    ui->tableWidget->setItem( row, 0, new QTableWidgetItem( user->p_info()->showName() ) );
    ui->tableWidget->setItem( row, 1, new QTableWidgetItem( user->p_info()->showSurname() ) );
    ui->tableWidget->setItem( row, 2, new QTableWidgetItem( user->p_info()->showClass() ) );
}

void userList::on_tableWidget_cellDoubleClicked(int row, int) {
    showUser userData( currentUsers.showUser( row ), allLoans, logUser );

    QObject::connect( &userData, SIGNAL( sendPath( loans, QString ) ), this, SLOT( recivePath( loans, QString ) ) );
    QObject::connect(  &userData, SIGNAL( eraseUser( QSharedPointer < account > ) ), this, SLOT( eraseUser( QSharedPointer < account > ) ) );
    QObject::connect( &userData, SIGNAL( sendUsername( QSharedPointer< account >, QString, QString ) ), this, SLOT( addUSername( QSharedPointer < account >, QString, QString ) ) );
    QObject::connect( this, SIGNAL( userResult(bool) ), &userData, SLOT( reciveResult(bool) ) );
...
void userList::recivePath( const loans &lends, const QString &path ){
...
void userList::on_addButton_clicked(){
    ...
    QObject::connect( &addAccount, SIGNAL( sendUser( QSharedPointer < account > ) ), this, SLOT( addUser( QSharedPointer < account > ) ) );
    QObject::connect( this, SIGNAL( userResult( bool ) ), &addAccount, SLOT( userResult( bool ) ) );

    addAccount.exec();
}

void userList::addUSername( const QSharedPointer < account > user, const QString &name, const QString &pass ){
