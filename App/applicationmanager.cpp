...
bool applicationManager::loadData() {
  if (io.firstStart()) {
    setup setUp;
    QObject::connect(&setUp, SIGNAL(sendPath(const QString &)), this,
                     SLOT(setDataPath(const QString &)));
    if (!setUp.exec()) return false;
  }
  ...
  if (!collector.p_accounts()->checkAdmin()) {
    ...
    QObject::connect(&needAdmin, SIGNAL(sendUser(QSharedPointer<account>)),
                     this, SLOT(addUser(QSharedPointer<account>)));
    QObject::connect(this, SIGNAL(userResult(bool)), &needAdmin,
                     SLOT(userResult(bool)));

    if (!needAdmin.exec()) return false;
...
void applicationManager::addUser(QSharedPointer<account> user) {
  if (!user->p_user().isNull()) {
    if (collector.p_accounts()
            ->showUserFromUsername(user->p_user()->showUser())
            .isNull()) {
...
void applicationManager::changeLanguage(const int &index) {
  ...
  if (index == 1) language = "en";
  if (index == 2) language = "it";
...
applicationManager::~applicationManager() { delete gui; }

void applicationManager::start() {
  gui = new mainWindow();

  QObject::connect(gui, SIGNAL(changeLanguage(int)), this,
                   SLOT(changeLanguage(int)));
  QObject::connect(this, SIGNAL(languageResult(bool)), gui,
                   SLOT(languageResult(bool)));
  ...
  QObject::connect(gui, SIGNAL(loginRequest(QString, QString)), this,
                   SLOT(loginRequest(QString, QString)));
  QObject::connect(this, SIGNAL(badLogin()), gui, SLOT(badLogin()));
  QObject::connect(gui, SIGNAL(publicationListRequest(bool)), this,
                   SLOT(openPublicationList(bool)));
  gui->show();
}
...
void applicationManager::loadLanguage(const QString &newLanguage) {
  ...
  if (newLanguage != currentLanguage) {
    if (newLanguage == "it") QLocale::setDefault(QLocale(QLocale::Italian));
    if (newLanguage == "en") QLocale::setDefault(QLocale(QLocale::English));
...
void applicationManager::loginRequest(const QString &name,
                                      const QString &password) {
  QSharedPointer<account> logUser =
      collector.p_accounts()->login(name, password);
  if (!logUser.isNull()) {
    if (logUser->p_permission()->showType() == 3) {
      gui->hide();
      adminMainWindow *adminPanel = new adminMainWindow(logUser);

      QObject::connect(adminPanel, SIGNAL(closeAdmin(adminMainWindow *)), this,
                       SLOT(closeAdmin(adminMainWindow *)));
      QObject::connect(adminPanel,
                       SIGNAL(openPublication(QSharedPointer<account>)), this,
                       SLOT(openPublication(QSharedPointer<account>)));
      QObject::connect(adminPanel, SIGNAL(openLoan(QSharedPointer<account>)),
                       this, SLOT(openLoan(QSharedPointer<account>)));
      QObject::connect(adminPanel, SIGNAL(saveData()), this, SLOT(saveData()));
      QObject::connect(this, SIGNAL(saveComplete(bool)), adminPanel,
                       SLOT(saveResult(bool)));
      QObject::connect(adminPanel, SIGNAL(uninstall()), this,
                       SLOT(removeLocalData()));
      QObject::connect(adminPanel, SIGNAL(openUser(QSharedPointer<account>)),
                       this, SLOT(openUser(QSharedPointer<account>)));
      ...
      QObject::connect(&userData, SIGNAL(sendPath(loans, QString)), this,
                       SLOT(writeCsv(loans, QString)));
      QObject::connect(&userData, SIGNAL(eraseUser(QSharedPointer<account>)),
                       this, SLOT(eraseUser(QSharedPointer<account>)));
      userData.exec();
      gui->show();
    }
  } else {
...
void applicationManager::openPublicationList(bool mode) {
  ...
  QObject::connect(&books, SIGNAL(sendPath(library, QString)), this,
                   SLOT(writeCsv(library, QString)));
  QObject::connect(&books, SIGNAL(sendPath(loans, QString)), this,
                   SLOT(writeCsv(loans, QString)));
...
void applicationManager::openPublication(
    const QSharedPointer<account> logUser) {
  publicationList BookList(collector.p_publications(), collector.p_loans(),
                           true, logUser);
  BookList.exec();
}
...
void applicationManager::openLoan(const QSharedPointer<account> logUser) {
  ...
  QObject::connect(&lendList, SIGNAL(sendPath(loans, QString)), this,
                   SLOT(writeCsv(loans, QString)));
...
void applicationManager::openUser(const QSharedPointer<account> logUser) {
  ...
  QObject::connect(&listUsers, SIGNAL(sendPath(loans, QString)), this,
                   SLOT(writeCsv(loans, QString)));
