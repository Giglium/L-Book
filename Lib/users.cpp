...
void users::addUser(const QSharedPointer<account> use) {
  if (!use.isNull()) {
    ...
    if ((!name.isNull() && checkIfUsernameExist(name->showUser())) ||
        !showUser(use->p_info()->showRegistrationNumber()).isNull())
      userCollection << use;
  }
}

bool users::addUsername(const QSharedPointer<account> use,
                        const QString &name) {
  if (checkIfUsernameExist(name)) use->p_user()->setUsername(name);
  return false;
}
...
int users::showUserNumber() const { return userCollection.size(); }

QSharedPointer<account> users::showUser(const int &position) const {
  if (position <= userCollection.size()) return userCollection.at(position);
...
QSharedPointer<account> users::showUser(const QString &rNumber) const {
  ...
  while (list.hasNext()) {
    ...
    if (supp->p_info()->showRegistrationNumber() == rNumber) return supp;
  }

  return QSharedPointer<account>();
}

QSharedPointer<account> users::showUser(
    const QSharedPointer<account> user) const {
...
bool users::checkAdmin() const {
  ...
  while (list.hasNext()) {
    ...
    if (user->p_permission()->showType() == 3) return true;
  }
...
users users::showUserFromNameSurname(const QString &nameSurname) const {
  ...
  while (list.hasNext()) {
    QSharedPointer<account> user = list.next();
    QStringList name = utility::toQStringList(
        user->p_info()->showName() + " " + user->p_info()->showSurname() + ",");
...
QSharedPointer<account> users::login(const QString &username,
                                     const QString &password) const {
  QSharedPointer<account> user = showUserFromUsername(username);
  if (!user.isNull() && user->p_user()->showPass() == password) return user;
  return QSharedPointer<account>();
}

void users::operator<<(const users &user) { addUser(user); }

void users::operator<<(const QSharedPointer<account> user) { addUser(user); }
