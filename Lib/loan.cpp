...
loan::loan( const QSharedPointer <account> accoun, const QSharedPointer < publication > publi, const QDate &startDate, const QDate &endDate, const bool &fin )
     :user( accoun ), title( publi ), startloan( startDate ), finish( fin ) {
    extendLoan( endDate );
}

loan::loan( const QSharedPointer < account > accoun, const QSharedPointer < publication > publi, const QDate &date )
      :user( accoun ), title( publi ), startloan( QDate::currentDate() ) ,finish(false) {
    extendLoan( date );
}

loan::loan( const QSharedPointer < account > accoun, const QSharedPointer < publication > publi, const int &duration )
      :user( accoun ), title( publi ), startloan( QDate::currentDate() ) ,finish(false) {
    extendLoan( duration );
}
...
QDate loan::showStartDate() const {
    return startloan;
}
...
QSharedPointer < account > loan::p_user() const {
    return user;
}

QSharedPointer < publication > loan::p_title() const {
    return title;
}

bool loan::isfinish() const {
    return finish;
}
