...
library::library() : publicationCollection() {}
...
void library::addPublication(const QSharedPointer<publication> publi) {
  if (!publi.isNull() && publi->showIdentification() != "" &&
      showPublication(publi->showIdentification()).isNull())
    publicationCollection << publi;
}

void library::removePublication(const QString &ID) {
  QList<QSharedPointer<publication> >::Iterator list =
      publicationCollection.begin();
...
QSharedPointer<publication> library::showPublication(
...
QSharedPointer<publication> library::showPublication(const QString &ID) const {
  ...
  while (list.hasNext()) {
    ...
    if (supp->showIdentification() == ID) return supp;
  }
...
QSharedPointer<publication> library::showPublication(
    const QSharedPointer<publication> publi) const {
...
library library::showPublicationsFromISSN(const QString &issn) const {
  ...
  while (list.hasNext()) {
    QSharedPointer<periodic> magazine =
        qSharedPointerDynamicCast<periodic>(list.next());
...
library library::showPublicationsFromISBN13(const QString &isbn) const {
  ...
  while (list.hasNext()) {
    ...
    if (publi->showISBN_13() == isbn) books << publi;
  }
...
void library::operator<<(const library &books) { addPublication(books); }
