...
void userinfo::getRegistrationNumber() {
  ...
  registration_number.append(
      surname[utility::randomNumber(surname.length() - 1)]);
...
userinfo::userinfo(const QString &nam, const QString &sur, const QDate &date,
                   const QString cla, const QString cNumber, const QString mail)
...
userinfo::userinfo(const QString &number, const QString &nam,
                   const QString &sur, const QDate &date, const QString cla,
                   const QString cNumber, const QString mail)
...
void userinfo::setName(const QString &nam) { name = nam; }

void userinfo::setSurname(const QString &sur) { surname = sur; }

void userinfo::setDate(const QDate &date) {
  birthdate.setDate(date.year(), date.month(), date.day());
}

void userinfo::setCellNumber(const QString &cell) { cellnumber = cell; }

void userinfo::setEmail(const QString &mail) { email = mail; }
...
QString userinfo::showRegistrationNumber() const { return registration_number; }
...
QDate userinfo::showBirthdate() const {
  if (birthdate.isValid()) return birthdate;
  return QDate();
}

QString userinfo::showCharBirthdate() const {
  if (birthdate.isValid()) return birthdate.toString("dd-MMM-yyyy");
  return QString("");
}
...
QString userinfo::showClass() const {
  if (clas.isEmpty()) return QString("");
  return clas.join(" ");
}

QStringList userinfo::showClassList() const { return clas; }

void userinfo::addClass(const QString &cla) {
  if (!cla.isEmpty()) clas << utility::toQStringList(cla);
}
...
void userinfo::removeAllClass() { clas.clear(); }
