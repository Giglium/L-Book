...
QString periodic::fromISSNtoISBN13(const QString &ISSN) {
  ...
  QStringList tmp =
      ISBN.split("", QString::SkipEmptyParts, Qt::CaseInsensitive);
...
unsigned int periodic::fromRoman(const QString &roman_number) {
  QStringList app = roman_number.toUpper().split("", QString::SkipEmptyParts,
                                                 Qt::CaseInsensitive);
...
periodic::periodic(const QString &ID, const unsigned int &year,
                   const unsigned int &num, const QString &title,
                   const QString &publishing, const QString &isbn,
                   const QDate &date, const QString subtitle,
                   const QString genre)
...
periodic::periodic(const QString &ID, const QString &year,
                   const unsigned int &num, const QString &title,
                   const QString &publishing, const QString &isbn,
                   const QDate &date, const QString subtitle,
                   const QString genre)
...
void periodic::setBirthYear(const unsigned int &year) { birthyear = year; }
...
void periodic::setNumber(const unsigned int &num) { number = num; }
...
QString periodic::showIdentification() const { return ISBN_13 + ID_number; }

int periodic::showBirthYear() const { return birthyear; }
...
int periodic::showNumber() const { return number; }

int periodic::showMonth() const { return release_date.month(); }

QString periodic::showCharMonth() const {
  return QDate::longMonthName(release_date.month());
}

QString periodic::showISSN() const { return fromISBN13toISSN(ISBN_13); }

QString periodic::showID_number() const { return ID_number; }
