...
void loans::addLoan(const QSharedPointer<loan> lend) {
  if (!lend.isNull()) loanCollection << lend;
}

void loans::addLoan(const QSharedPointer<account> user,
...
void loans::addLoan(const QSharedPointer<account> user,
                    QSharedPointer<publication> publi, const int &date) {
  addLoan(QSharedPointer<loan>(new loan(user, publi, date)));
}

int loans::showLoanNumber() const { return loanCollection.size(); }

QSharedPointer<loan> loans::showLoan(const int &position) const {
  if (position <= loanCollection.size()) return loanCollection.at(position);
...
loans loans::showDeadlineLoan(const int &deadline) const {
  ...
  while (list.hasNext()) {
    QSharedPointer<loan> lend = list.next();
    if (lend->showDayToEnd() < deadline && !lend->isFinish()) lends << lend;
  }
...
loans loans::showExpiredLoan() const {
  ...
  while (list.hasNext()) {
    QSharedPointer<loan> lend = list.next();
    if (lend->showDayToEnd() < 0) lends << lend;
  }
...
loans loans::publicationLoans(const QString &ID) {
  ...
  while (list.hasNext()) {
    QSharedPointer<loan> lend = list.next();
    if (lend->p_title()->showIdentification() == ID) lends << lend;
  }
...
loans loans::usernameLoans(const QString &user) const {
  ...
  while (list.hasNext()) {
    QSharedPointer<loan> lend = list.next();
    if (lend->p_user()->p_user()->showUser() == user) lends << lend;
  }
...
loans loans::surnameLoans(const QString &user) const {
  ...
  while (list.hasNext()) {
    QSharedPointer<loan> lend = list.next();
    if (lend->p_user()->p_info()->showSurname() == user) lends << lend;
  }
...
loans loans::professorLoans() const {
  ...
  while (list.hasNext()) {
    QSharedPointer<loan> lend = list.next();
    if (lend->p_user()->p_permission()->showType() == 2) pLoans << lend;
  }
...
loans loans::studentLoans() const {
  ...
  while (list.hasNext()) {
    QSharedPointer<loan> lend = list.next();
    if (lend->p_user()->p_permission()->showType() == 1) sLoans << lend;
  }
...
loans loans::publicationLoans(const QString &isbn) const {
  ...
  while (list.hasNext()) {
    QSharedPointer<loan> lend = list.next();
    if (lend->p_title()->showISBN_13() == isbn) lends << lend;
  }
...
bool loans::makeLoan(const QSharedPointer<account> user,
                     const QSharedPointer<publication> publi,
                     const int &duration) {
  QSharedPointer<account_permission> userPermission = user->p_permission();
  QSharedPointer<publication_permission> publiPermission =
      publi->p_permission();

  if (userPermission && publiPermission) {
    if (userPermission->showFreedomLoan() &&
        publiPermission->showFreedomCopy()) {
...
void loans::removeAllUserLoan(const QSharedPointer<account> user) {
  ...
  while (list.hasNext()) {
    QSharedPointer<loan> lend = list.next();
    if (lend->p_user()->p_info()->showRegistrationNumber() ==
        user->p_info()->showRegistrationNumber())
      removeLoan(lend);
  }
}
...
void loans::operator<<(const QSharedPointer<loan> lend) { addLoan(lend); }

void loans::operator<<(const loans &lend) { addLoan(lend); }
