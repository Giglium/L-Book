...
QString utility::encrypt(const QString &pass) {
...
QStringList utility::toQStringList(const QString &string) {
  ...
  if (string.contains(","))
    return stringList << string.split(",", QString::SkipEmptyParts,
                                      Qt::CaseInsensitive);
...
bool utility::checkAllInteger(const QString &string) {
  for (int i = 0; i < string.size(); ++i) {
    if (string[i].isLetter()) return false;
  }
...
bool utility::checkISSN(QString issn) {
  ...
  if (issn.size() == 8) {
    QStringList stringList =
        issn.split("", QString::SkipEmptyParts, Qt::CaseInsensitive);
    QStringListIterator list(stringList);
    ...
    if (sum == 10 && issn.at(7).toUpper() == 'X') return true;

    if (sum == issn.at(7).digitValue()) return true;
  }
  return false;
}

bool utility::checkString(const QString firstString, const QString secondString,
  ...
  return utility::checkString(firstStringList, secondoStringList,
                              caseSensitive);
}

bool utility::checkString(const QStringList firstStringList,
                          const QString string, const bool caseSensitive) {
...
bool utility::checkString(const QString string,
                          const QStringList secondStringList,
                          const bool caseSensitive) {
...
bool utility::checkString(const QStringList firstStringList,
  ...
  while (list.hasNext()) {
    ...
    QStringList secondList =
        firstStringList.filter(string, Qt::CaseInsensitive);
