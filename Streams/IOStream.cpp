...
QSharedPointer < loans > IOStream::loadLoans( const QSharedPointer < library > libraryList, const QSharedPointer < users > userList, const QString &path ) {
...
void IOStream::writeJsonFile( const QJsonDocument &dataObject, const QString &path ) const  {
    QFile file;
    file.setFileName( path );
...
QSharedPointer < account > IOStream::fromJsontoUser( const QJsonObject &dataObject ) {
    QSharedPointer < userinfo > info ( new userinfo( dataObject["registration"].toString() , dataObject["name"].toString(), dataObject["surname"].toString(), QDate::fromString( dataObject["date"].toString(), "yyyy/MM/dd" ), dataObject["class"].toString(), dataObject["cellnumber"].toString(), dataObject["email"].toString() ));
    QSharedPointer <account> user( new account ( info ) );

    if( !dataObject["username"].isNull() && dataObject["username"].toString() != "" ) {
        user->setUser( dataObject["username"].toString(), dataObject["password"].toString() );
    }

    if( !dataObject["type"].isNull() ) {
        user->setPermission( dataObject["type"].toInt(), dataObject["maxloan"].toInt() );
    }
...
QSharedPointer < publication > IOStream::fromJsontoPubblication( const QJsonObject &dataObject ) {

    QSharedPointer < publication_permission > permission = QSharedPointer < publication_permission >();

    if( !dataObject["permissionType"].isNull() )
        permission = QSharedPointer < publication_permission > ( new publication_permission( dataObject["permissionType"].toInt(), dataObject["copynumber"].toInt() ) );



    if( dataObject["type"].toString() == "book" ){
       QSharedPointer < book > tome ( new book( dataObject["author"].toString(), dataObject["plot"].toString() , dataObject["title"].toString(), dataObject["publishing"].toString(), dataObject["isbn"].toString(), dataObject["subtitle"].toString(), dataObject["genre"].toString(), QDate::fromString( dataObject["releaseDate"].toString(), "yyyy/MM/dd" ), dataObject["place"].toString(), dataObject["collection"].toString(), dataObject["edition"].toInt(), dataObject["numberOfPage"].toInt(), dataObject["year"].toInt() ) );

       if( !dataObject["permissionType"].isNull() )
           tome->setPermission( permission );

       return tome;
    }
    if( dataObject["type"].toString() == "periodic" ){
        QSharedPointer < periodic > magazine ( new periodic( dataObject["ID"].toString(), dataObject["birthyear"].toInt(), dataObject["number"].toInt(), dataObject["title"].toString(), dataObject["publishing"].toString(), dataObject["isbn"].toString(), QDate::fromString( dataObject["releaseDate"].toString(), "yyyy/MM/dd" ), dataObject["subtitle"].toString(), dataObject["genre"].toString() ) );
        if( !dataObject["permissionType"].isNull() )
...
QSharedPointer < loan > IOStream::fromJsontoLoan( const QJsonObject &dataObject, const QSharedPointer < library > libraryList, const QSharedPointer < users > userList ) const {
    ...
    if( !dataObject["usersID"].isNull() )
     ...
     if( !dataObject["publicationID"].isNull() )
        loanPublication = libraryList->showPublication( dataObject["publicationID"].toString() );
    ...
    return QSharedPointer < loan >( new loan( loanUser, loanPublication, QDate::fromString( dataObject["startDate"].toString(), "yyyy/MM/dd" ), QDate::fromString( dataObject["endDate"].toString(), "yyyy/MM/dd" ), dataObject["finish"].toBool() ) );
}

void IOStream::writeJsonfromUser( const QSharedPointer < users > userList, const QString &path ) const {
...
void IOStream::writeJsonfromLibrary( const QSharedPointer < library > publicationList, const QString &path ) const {
    QJsonArray dataArray;
...
void IOStream::writeJsonfromLoans( const QSharedPointer < loans > loanList, const QString &path ) const {
    QJsonArray dataArray;
...
QJsonObject IOStream::toJson( const QSharedPointer < publication > publicationList ) const {
    QJsonObject dataObject;
    QSharedPointer < publication_permission > permission = publicationList->p_permission();
    ...
    dataObject["releaseDate"] = publicationList->showReleaseDate().toString("yyyy/MM/dd");
    ...
    QSharedPointer < periodic > periodical = qSharedPointerDynamicCast < periodic > ( publicationList );
...
QJsonObject IOStream::toJson( const QSharedPointer < loan > loanList ) const {
      ...
      if( !loanList->p_user().isNull() )
        dataObject["usersID"] = loanList->p_user()->p_info()->showRegistrationNumber();
...
IOStream::IOStream()
         :localPath( QStandardPaths::writableLocation( QStandardPaths::AppLocalDataLocation ) ) {
...
bool IOStream::setSharedDir() {
    return setSharedDir( localPath.absolutePath() );
}
...
void IOStream::loadsettings() {
    QJsonObject dataObject = readJsonFile( utility::fusePath( localPath, "settings.json" ) );
...
dataCollector IOStream::loadData( const QString &path) {
   QSharedPointer < users > usersList  = loadUsers( utility::fusePath( path, "users.json" ) );

   QSharedPointer < library > libraryList = loadLibrary( utility::fusePath( path, "library.json" ) );

   QSharedPointer < loans > loansList = loadLoans( libraryList, usersList, utility::fusePath( path, "loans.json" ) );
...
void IOStream::saveSettings() {
    ...
    writeJsonFile( QJsonDocument( dataObject ), utility::fusePath( localPath, "settings.json" ) );
}
...
void IOStream::saveData( const dataCollector &data, const QString &path ) const {
    ...
    writeJsonfromLibrary( data.p_publications(), utility::fusePath( path, "library.json" ) );
...
bool IOStream::switchTranslator( QTranslator &translator ) const{
 ...
 if( translator.load( utility::fusePath( path, QString("%1.qm").arg( newLanguage ) ) ) ){
...
void IOStream::toCsv( const QSharedPointer < loan > lend, QTextStream &out ) {
    out<<lend->p_user()->p_info()->showName()<<";"<<lend->p_user()->p_info()->showSurname()<<";"<<lend->p_user()->p_info()->showClass()<<";"<<lend->p_title()->showTitle()<<";"<<lend->showCharStartDate()<<";"<<lend->showCharEndDate()<< "\n";
}

void IOStream::toCsv( const QSharedPointer< publication > publicatio, QTextStream &out ){
    ...
    if( !magazine.isNull() ){
        out<<magazine->showNumber()<<";"<<magazine->showGenre()<<";"<<magazine->showISSN()<<";";
    }

    out<<publicatio->showISBN_13()<<"\n";
}

void IOStream::removeLocal() {
    localPath.removeRecursively();
}
